...
var pool = module.exports = function pool(options, authorizeFn){
    ...
    function SubmitAuxBlock(aux, headerBuffer, coinbaseBuffer, blockHash, callback) {
        ...
        if (_this.options.coin.getblocktemplate == "POS") {
            var str = auxpow.toString('hex');
            var strL = str.length;
            var res = str.substr(0, 8);
            var res1 = str.substr(16, strL);
            //var strL1 = cut.length;
            //var res2 = cut.substr(0, (strL1 - 24));
            //var res3 = cut.substr((strL1 - 16), (strL1 - 8));
            //var testVar = res+res2+res3;
            var testVar = res+res1;
            console.log("_this.auxes[aux].rpcData == "+JSON.stringify(_this.auxes[aux].rpcData)+"\n");
            console.log(str+"\n");
            console.log(testVar+"\n");
            _this.auxes[aux].daemon.cmd('getauxblock', [_this.auxes[aux].rpcData.hash, testVar], function(results) {
            ...
            _this.auxes[aux].daemon.cmd('getauxblock', [_this.auxes[aux].rpcData.hash, auxpow.toString('hex')], function(results) {
                for (var i = 0; i < results.length; i++) {
                    var result = results[i];
                    if(result.error && result.error !== null) {
                        emitErrorLog('Failed to submit potential auxiliary block: ' + JSON.stringify(result.error));
